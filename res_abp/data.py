...
def main():
    for infile in glob.glob(os.path.join('','better*')):
        ...
        for filtered_item in ping_ack_array:#filter(filterMoreThanOneTry, ping_ack_array):
            print "%d, %d, %d" % (filtered_item["seq"], filtered_item["resend"] - filtered_item["radiofail"], filtered_item["radiofail"])
